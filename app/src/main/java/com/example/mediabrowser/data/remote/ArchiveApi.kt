...
interface ArchiveApi {
    ...
    /**
     * Looks up MANY tags in ONE request via the `names` list-param
     * (space-separated exact names). This is how a whole post's tag set gets
     * categorized in a single round trip instead of one request per tag.
     * The param can occasionally miss entries, so callers must per-tag-fallback
     * anything absent from the response (see MediaRepositoryImpl).
     */
    @GET("index.php?page=dapi&s=tag&q=index&json=1")
